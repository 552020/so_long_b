...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/27 20:34:56 by slombard          #+#    #+#             */
/*   Updated: 2023/07/27 20:38:53 by slombard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_info(t_game *game)
{
	mlx_put_string(game->mlx, "MOVES:",
		(game->width - (game->width - 1)) * PIXELS, 
		(game->height - 0.8) * PIXELS);
	mlx_image_to_window(game->mlx, game->img->collectible,
		(game->width - (game->width - 1)) * PIXELS, 
		(game->height - 0.6) * PIXELS);
	mlx_put_string(game->mlx, ":", (game->width - (game->width - 1.8)) 
		* PIXELS, (game->height - 0.5) * PIXELS);
	render_moves(game);
